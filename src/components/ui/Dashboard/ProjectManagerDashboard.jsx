...
const LeaveRequests = () => {
    ...
    const userLeaveRequests = leaveRequests.filter(request => (request.managerId === user.id && request.status === "PENDING"));
                        ...
                        <table className="w-full text-left table-fixed">
                            ...
                            <tbody>
                                {userLeaveRequests.length === 0 ?
                                    <tr className="text-center"><td className="p-5" colSpan={7}>Their isn't any Pending Requests</td></tr> :
                                        ...
                                        return (
                                            <tr key={request.id} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className={`py-2 flex justify-center`}>
                                                    <span className={`text-white rounded-full p-[5px_25px] text-center border border-black bg-yellow-500`}>
                                                        {request.status}
                                                    </span>
                                                </td>
                                                <td>
